...
class MasterDataClient:
    ...
    async def get_bed(self, bed_id: str, jwt_token: str = "") -> Optional[Dict[str, Any]]:
        """Retrieves bed details for category mapping."""
        request = master_data_pb2.BedQuery(bed_id=bed_id)
        metadata = (("authorization", f"Bearer {jwt_token}"),) if jwt_token else ()
        try:
